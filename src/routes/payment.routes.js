  ...
  try {
    ...
    if (!razorpay) {
      return res.status(503).json({
        success: false,
        message: 'Razorpay API keys not configured. Please set RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET in backend .env.',
      });
    }
...
router.post('/verify', optionalAuthenticate, async (req, res) => {
  try {
    ...
    if (orderId) {
      if (req.user) {
        await pool.query(
          "UPDATE orders SET paymentStatus = 'PAID', razorpayPaymentId = ?, razorpayOrderId = ?, status = 'CONFIRMED', updatedAt = NOW() WHERE id = ? AND userId = ?",
          [razorpay_payment_id, razorpay_order_id, orderId, req.user.id]
        );
      } else {
        await pool.query(
          "UPDATE orders SET paymentStatus = 'PAID', razorpayPaymentId = ?, razorpayOrderId = ?, status = 'CONFIRMED', updatedAt = NOW() WHERE id = ?",
